...
metaDataFile = 'ipMetaData.csv'

event_date = datetime.datetime(2019, 6, 7)
with open(metaDataFile) as csvIPData:
    ipMetaData = list(csv.reader(csvIPData))

def get_src_index(ip1):
    for k in range(50000):
        if ipMetaData[k][0] == ip1:
            return k
def get_dst_index(ip2):
    for k in range(50000):
        if ipMetaData[k][0] == ip2:
            return k
...
def fetch_metadata(fourtuple,j):
    ...
    #p = j//4000
    p = get_src_index(src_ip)
    #q = 12500 + (j%4000)
    q = get_dst_index(dst_ip)
    if p and q:
      ...
      modelKey = modelkey_oid_range[j]
      collector_id = collector_id_range[j]
...
if __name__ == '__main__':
    ...
    with open('4TupleFromDb.csv', 'r') as f:
        ...
        max_tuples = ((files_created + 50) * s) / t
        for eachTuple in csv.reader(f):
            if (j < (files_created * s) / t):
                ...
                for version in range(t):
                    ...
                    # For druid timestamp needs to be millis
                    active_timestamp = int(event_datetime.timestamp()) * 1000
                    ...
                    x.insert(1, active_timestamp)
                    x.insert(95, sch_version)
                    x.insert(96, lastactivity_range[j])
                    x.insert(97, active_timestamp + lastactivity_range[j])
                    x = x + all_metrics_active
                    events.append(x)
